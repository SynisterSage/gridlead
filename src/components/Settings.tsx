...
const Settings: React.FC<SettingsProps> = ({ onLogout, profile, userName, userEmail, userAvatarUrl, agencyName, authProvider }) => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'profile':
        if (!animReady) {
          // animate usage bars once when profile tab is shown
          setTimeout(() => setAnimReady(true), 150);
        }
        return (
          <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
            ...
            <div ref={billingRowRef} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[1.75rem] p-6 md:p-8 shadow-sm space-y-6">
                    ...
                    {leadLimit ? (
                      <>
                        <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 overflow-hidden">
                          <div
                            className={`h-3 ${leadBarColor} transition-all duration-500`}
                            style={{ width: animReady ? `${leadsPct}%` : '0%' }}
                          />
                        </div>
                  ...
                  <div className="space-y-2">
                    {seatLimit ? (
                      <>
                        <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 overflow-hidden">
                          <div
                            className={`h-3 ${seatLimit && seatsPct >= 100 ? 'bg-rose-500' : seatsPct >= 80 ? 'bg-amber-400' : 'bg-sky-500'} transition-all duration-500`}
                            style={{ width: animReady ? `${seatsPct}%` : '0%' }}
                          />
                        </div>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <button onClick={() => setShowUpgradeModal(true)} className="flex-1 min-w-[180px] px-4 py-2.5 bg-[#0f172a] text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-slate-800 transition-all shadow-sm">
                  Upgrade
                </button>
                <button disabled className="flex-1 min-w-[180px] px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-[10px] font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400 bg-transparent cursor-not-allowed">
                  Manage subscription
                </button>
              </div>
            </div>
