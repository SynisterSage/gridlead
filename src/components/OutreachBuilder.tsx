              ...
              {currentLead ? (
                <div className="flex-1 flex flex-col min-h-0 animate-in fade-in slide-in-from-right-2 duration-300">
                    ...
                    <div className="max-w-4xl mx-auto space-y-8">
                      {['sent', 'responded', 'won'].includes(currentLead.status) ? (
                        <div className="space-y-8">
                          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-8 shadow-sm border-l-4 border-l-blue-500">
                             <div className="flex items-center justify-between mb-6">
                               <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                 <Mail size={14} className="text-blue-500" /> Outbound Sequence
                               </span>
                             </div>
                             <h4 className="text-lg font-extrabold text-[#0f172a] dark:text-white mb-4 leading-tight">Subject: {currentLead.draftSubject}</h4>
                             <p className="text-[14px] text-slate-600 dark:text-slate-300 leading-relaxed font-medium whitespace-pre-wrap">{currentLead.draftBody}</p>
                          </div>
                          {currentLead.status === 'responded' && (
                            <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30 rounded-3xl p-8 shadow-xl border-l-4 border-l-emerald-500 self-end ml-10">
                               <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest flex items-center gap-2 mb-6">
                                  <MessageSquare size={14} /> Incoming Reply
                               </span>
                               <p className="text-xl text-emerald-950 dark:text-emerald-50 font-black italic">"Thanks for reaching out. We've actually been looking for someone to audit our performance. Are you free for a call?"</p>
                            </div>
                          )}
                        </div>
                      ) : (
                  ...
                  {currentLead.status === 'responded' && (
                    <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 bg-gradient-to-t from-white dark:from-slate-900 via-white dark:via-slate-900 to-transparent pointer-events-none z-20">
                      <div className="max-w-4xl mx-auto pointer-events-auto">
                        <button onClick={() => onUpdateLead(currentLead.id, { status: 'won' })} className="w-full h-16 bg-amber-500 text-white rounded-2xl text-[12px] font-black uppercase tracking-widest flex items-center justify-center gap-4 hover:bg-amber-600 transition-all shadow-2xl active:scale-95 ring-4 ring-amber-500/10">
                          <Trophy size={20} /> Move to Closed Won
                        </button>
                      </div>
                    </div>
                  )}
